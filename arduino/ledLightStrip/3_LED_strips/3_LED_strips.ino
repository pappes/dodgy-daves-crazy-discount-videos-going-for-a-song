...
Adafruit_NeoPixel strip1 = Adafruit_NeoPixel(50, LEDSTRIP1, NEO_GRB + NEO_KHZ800);//78
Adafruit_NeoPixel strip2 = Adafruit_NeoPixel(50, LEDSTRIP2, NEO_GRB + NEO_KHZ800);//78
Adafruit_NeoPixel strip3 = Adafruit_NeoPixel(50, LEDSTRIP3 , NEO_GRB + NEO_KHZ800);//78
Adafruit_NeoPixel strip5 = Adafruit_NeoPixel(50, LEDSTRIP5, NEO_GRB + NEO_KHZ800);//1 x 78, 2 x 156, 3 x 234
Adafruit_NeoPixel strip6 = Adafruit_NeoPixel(50, LEDSTRIP6, NEO_GRB + NEO_KHZ800);//5m = 300
...
void initilaise_led_strip(byte strip_num, byte strip_pin, byte strip_button, Adafruit_NeoPixel &strip_ref) {
  ...
  pinMode(strip_button,INPUT_PULLUP);
  //pinMode(strip_button, INPUT);
  strip1.begin();
  led_strips[strip_num] = &strip_ref;
}
void reinitilaise_led_strip(byte strip) {
  if (nullptr == led_strips[strip]) return;
  if (1 == led_strips[strip]->numPixels()) led_strips[strip]->updateLength(led_strip_length[strip]);
}
